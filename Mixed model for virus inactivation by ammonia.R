...
library(ggplot2)
#matrix
matrix$Matrices <- factor(matrix$Matrices, levels = c("Water/buffer", "Urine", 
                                                      "Feces/fecal sludge", "Digested sludge",
...
library (lme4)
lmer1a <- lmer (log (k) ~ log_NH3+(1|Virus), data=cluster1n)
lmer2a <- lmer (log (k) ~ log_NH3+Temp+Matrices+(1|Virus), data=cluster1n)
lmer3a <- lmer (log (k) ~ log_NH3+Matrices+(1|Virus), data=cluster1n)
lmer4a <- lmer (log (k) ~ log_NH3+Temp+(1|Virus), data=cluster1n)
lmer5a <- lmer (log (k) ~ log_NH3+log(Temp)+log(Matrices)+(1|Virus), data=cluster1n)
lmer6a <- lmer (log (k) ~ log_NH3+log(Temp)+(Matrices)+(1|Virus), data=cluster1n)
lmer7a <- lmer (log (k) ~ log_NH3+Temp+log(Matrices)+(1|Virus), data=cluster1n)
...
library (car)
lmer8a <- lmer (log (k) ~ log_NH3+Temp+Matrices*log_NH3+(Matrices)+(1|Virus), data=cluster1n)
vif(lmer8a) #multicollinear
lmer9a <- lmer (log (k) ~ log_NH3+Temp+Temp*log_NH3+(Matrices)+(1|Virus), data=cluster1n)
vif(lmer9a) #multicollinear
lmer10a <- lmer (log (k) ~ log_NH3+Temp*Matrices+Temp+(Matrices)+(1|Virus), data=cluster1n)
vif(lmer10a)
AIC(lmer8a,lmer9a,lmer10a)

#checking if pH worth to be included or not
lmer11a <- lmer (log (k) ~ log_NH3+Temp+pH+(Matrices)+(1|Virus), data=cluster1n)
lmer12a <- lmer (log (k) ~ log_NH3+pH+(1|Virus), data=cluster1n)
lmer13a <- lmer (log (k) ~ log_NH3+Matrices+pH+(1|Virus), data=cluster1n)
lmer14a <- lmer (log (k) ~ log_NH3+Temp+pH+(1|Virus), data=cluster1n)
lmer15a <- lmer (log (k) ~ log_NH3+log(Temp)+Matrices+pH+(1|Virus), data=cluster1n)
lmer16a <- lmer (log (k) ~ log_NH3+Temp+log(Matrices)+pH+(1|Virus), data=cluster1n)
...
p1 +
  annotate("text", x = -0.5, y = 2.5, label = eq_text, size=7, color = "black")+
  theme (axis.title = element_text(size = 22, face = "bold"),
         axis.text = element_text(size = 18))+
  labs (x="Ammonia Activity (mM)", y = "Rate Constant (L mol⁻¹ day⁻¹)")

#cluster2
coefs2 <- fixef(lmer2)

eq_text2 <- paste0("Y = ", "0.453", "X", "-5.87", "\n",
                   "R² = 0.419")

p2 <- effect_plot(data=cluster2n, model = lmer2, pred = log_NH3, interval = TRUE, plot.points = TRUE, 
                  jitter = 0.05, colors = "#21918c")
p2 +
  annotate("text", x = 3, y = 3, label = eq_text2, size=7, color = "black")+
  theme (axis.title = element_text(size = 22, face = "bold"),
         axis.text = element_text(size = 18))+
  labs (x="Ammonia Activity (mM)", y = "Rate Constant (L mol⁻¹ day⁻¹)")
...
p3 +
  annotate("text", x = 2.5, y = 2.5, label = eq_text3, size=7, color = "black")+
  theme (axis.title = element_text(size = 22, face = "bold"),
         axis.text = element_text(size = 18))+
  labs (x="Ammonia Activity (mM)", y = "Rate Constant (L mol⁻¹ day⁻¹)")
